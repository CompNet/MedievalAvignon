# Random model to study how the absence of position information affects the 
...
# source("res/analysis/model_remove_positions.R")
###############################################################################
# load other scripts
...
{	del.rate <- del.rates[d]
	...
	# randomly remove some positions
	tlog(10,"Removing positions")
	idx <- sample(gorder(g), size=round(del.rate*gorder(g)))
	g1 <- g
	if(length(idx)>0)
	{	g1 <- set_vertex_attr(g1,  name=COL_LOC_X, index=idx, value=rep(NA,length(idx)))
		g1 <- set_vertex_attr(g1, name=COL_LOC_Y, index=idx, value=rep(NA,length(idx)))
	}
	...
	coords <- cbind(vertex_attr(graph=g1, name=COL_LOC_X), vertex_attr(graph=g1, name=COL_LOC_Y))
	...
	gvals <- distances(graph=g1, mode="all", v=idx0, to=idx0)
	diag(gvals) <- NA
	...
	meas <- MEAS_DEGREE
	vals <- igraph::degree(graph=g1, mode="all", v=idx0)
