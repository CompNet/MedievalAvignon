...
#	last <- last + nrow(info.fix)
	...
	{	tlog(4,"Splitting linear and surface vertices")
		...
		{	tlog(8,"Splitting vertex '",info.split[r,COL_STREET_ID],"' (row #",r,"/",nrow(info.split),")")
			...
			street.flag <- !is.na(info.all[idx,COL_STREET_LENGTH]) && is.numeric(split.surf) && info.all[idx,COL_STREET_LENGTH]>=split.surf
...
#	data[data[,COL_CONF_LOC_NORM]==VAL_CONF_TYPE_EXTERIEUR, COL_CONF_LOC_NORM] <- VAL_CONF_TYPE_MISC
	...
	# possibly remove certain leaves attached to a single artificial neighbor
	if(is.logical(split.surf) && split.surf || is.numeric(split.surf))
...
#		write.table(x=layout, file=lay.file, sep="\t", row.names=FALSE, col.names=TRUE)
	...
	if(is.logical(split.surf) && !street.ablation)
	{	# record as graphml file
		...
		custom.gplot(g=g1, file=paste0(plot.file,"_algo"), axes=FALSE, rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), vertex.label.cex=0.1, size.att=8)	# edge.arrow.mode=0, 
