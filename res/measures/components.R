...
#		idx <- which(cmp$csize >= 0.1*gorder(g))	# keep only the components containing at least 10% of the nodes
		...
		custom.gplot(g=g, col.att=fname, cat.att=TRUE, file=file.path(comp.folder,paste0(fname,"_graph")), size.att=2, edge.arrow.mode=0)
		...
		{	# plot subgraph
			...
			custom.gplot(g=g2, file=file.path(sep.folder,paste0("component_",i)), size.att=2, edge.arrow.mode=0)
			g1 <- g; V(g1)$x <- V(g1)$x2; V(g1)$y <- V(g1)$y2; E(g1)$weight <- 0.5; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1)
			V(g1)$label <- paste(vertex_attr(g1,name=COL_LOC_ID), get.location.names(g1),sep="_")
