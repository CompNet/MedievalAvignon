...
#	modes <- c(MEAS_MODE_UNDIR, MEAS_MODE_DIR)
	...
	{	mode <- modes[i]
		...
		custom.gplot(g=g1, col.att=fname, col.att.cap=MEAS_LONG_NAMES[MEAS_BETWEENNESS], file=paste0(plot.file,"_algo"), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
	}
