...
#	modes <- c(MEAS_MODE_UNDIR, MEAS_MODE_DIR)
	...
	{	tlog(2,"Computing vertex connectivity: mode=",mode)
		...
		{	# export CSV with average connectivity
			...
			custom.gplot(g=g1, col.att=paste0(fname,"_avg"), file=file.path(connectivity.folder,paste0(fname,"_avg_graph_kk")), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
		}
