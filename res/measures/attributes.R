...
{	tlog(2,"Computing nodal attribute stats")
	...
	{	# get values only for real-estate vertices
		...
		custom.gplot(g=g00, col.att=attr, cat.att=TRUE, color.isolates=TRUE, file=paste0(plot.file,"_lambert"), size.att=2, edge.arrow.mode=0)
		V(g00)$x <- V(g00)$x2; V(g00)$y <- V(g00)$y2; E(g00)$weight <- 0.5; g00 <- delete_edge_attr(g00, LK_TYPE); g00 <- simplify(g00)
		V(g00)$label <- paste(vertex_attr(g00,name=COL_LOC_ID), get.location.names(g00),sep="_")
		custom.gplot(g=g00, col.att=attr, cat.att=TRUE, color.isolates=TRUE, file=paste0(plot.file,"_kk"), rescale=FALSE, xlim=range(V(g00)$x), ylim=range(V(g00)$y), edge.arrow.mode=0, vertex.label.cex=0.1)
	...
	{	# get values only for real-estate vertices
		...
		custom.gplot(g=g00, col.att=attrc, cat.att=TRUE, color.isolates=TRUE, file=paste0(plot.file,"_lambert"), size.att=2, edge.arrow.mode=0)
		V(g00)$x <- V(g00)$x2; V(g00)$y <- V(g00)$y2; E(g00)$weight <- 0.5; g00 <- delete_edge_attr(g00, LK_TYPE); g00 <- simplify(g00)
		V(g00)$label <- paste(vertex_attr(g00,name=COL_LOC_ID), get.location.names(g00),sep="_")
		custom.gplot(g=g00, col.att=attrc, cat.att=TRUE, color.isolates=TRUE, file=paste0(plot.file,"_kk"), rescale=FALSE, xlim=range(V(g00)$x), ylim=range(V(g00)$y), edge.arrow.mode=0, vertex.label.cex=0.1)
		...
		{	# binarize tags
			...
			V(g00)$label <- rep(NA, gorder(g00))
			custom.gplot(g=g00, col.att=att_name, col.att.cap=paste0(LONG_NAME[attr]," : ",uval), cat.att=TRUE, color.isolates=TRUE, file=paste0(plot.file,"_lambert"), size.att=2, edge.arrow.mode=0)
			V(g00)$x <- V(g00)$x2; V(g00)$y <- V(g00)$y2; E(g00)$weight <- 0.5; g00 <- delete_edge_attr(g00, LK_TYPE); g00 <- simplify(g00)
			V(g00)$label <- paste(vertex_attr(g00,name=COL_LOC_ID), get.location.names(g00),sep="_")
			custom.gplot(g=g00, col.att=att_name, col.att.cap=paste0(LONG_NAME[attr]," : ",uval), cat.att=TRUE, color.isolates=TRUE, file=paste0(plot.file,"_kk"), rescale=FALSE, xlim=range(V(g00)$x), ylim=range(V(g00)$y), edge.arrow.mode=0, vertex.label.cex=0.1)
		}
	}
...
#	num.data[which(is.na(num.data))] <- VAL_UNK
	...
	{	for(i in 1:ncol(num.data))
		{	attr <- colnames(num.data)[i]
			...
			custom.gplot(g=g00, col.att=attr, cat.att=FALSE, color.isolates=TRUE, file=paste0(plot.file,"_lambert"), size.att=2, edge.arrow.mode=0)
			V(g00)$x <- V(g00)$x2; V(g00)$y <- V(g00)$y2; E(g00)$weight <- 0.5; g00 <- delete_edge_attr(g00, LK_TYPE); g00 <- simplify(g00)
			V(g00)$label <- paste(vertex_attr(g00,name=COL_LOC_ID), get.location.names(g00),sep="_")
			custom.gplot(g=g00, col.att=attr, cat.att=FALSE, color.isolates=TRUE, file=paste0(plot.file,"_kk"), rescale=FALSE, xlim=range(V(g00)$x), ylim=range(V(g00)$y), edge.arrow.mode=0, vertex.label.cex=0.1)
		}
	}
	else
