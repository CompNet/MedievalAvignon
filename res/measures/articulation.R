...
{	# get the stat table
	...
	{	# repeat until no more articulation point
		...
		custom.gplot(g=g1, col.att=MEAS_ARTICULATION, col.att.cap=MEAS_LONG_NAMES[MEAS_ARTICULATION], file=paste0(plot.file,"_algo"), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
