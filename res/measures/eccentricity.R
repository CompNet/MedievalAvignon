...
#############################################################
analyze.net.eccentricity <- function(g, out.folder, fast)
{	# get the stat table
...
#				all_shortest_paths(graph=g, from=idx[r,1], to=idx[r,2], mode=if(mode==MEAS_MODE_DIR) "in" else "all")$res
			...
			{	tlog(6,"Plotting diameter path ",pp,"/",length(diam.paths)," in '",file.path(diameter.folder,"xxxx",paste0(MEAS_DIAMETER,"_",mode,"_graph_",pp)),"'")
				...
				custom.gplot(g=g1, paths=diam.paths[[pp]], file=file.path(diameter.folder,"lambert",paste0(MEAS_DIAMETER,"_",mode,"_graph_",pp)), asp=1, size.att=2, edge.arrow.mode=0, vertex.label.cex=0.1, rescale=FALSE)
				...
				{	tlog(8,"Plotting the ",length(diam.paths[[pp]])," variants of this diameter")			
					...
					{	tlog(10,"Plotting variant ",p,"/",length(diam.paths[[pp]]))
						...
						{	#V(g)$label <- rep(NA,gorder(g))
							...
							V(g)$label <- paste(vertex_attr(g,name=COL_LOC_ID), get.location.names(g),sep="_")
							g1 <- g; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1); g1 <- rescale.coordinates(g1)
							custom.gplot(g=g1, paths=diam.paths[[pp]][[p]], file=file.path(diameter.folder,"lambert",paste0(MEAS_DIAMETER,"_",mode,"_graph_",pp,"_",q)), asp=1, size.att=2, edge.arrow.mode=0, vertex.label.cex=0.1, rescale=FALSE)
							g1 <- g; V(g1)$x <- V(g1)$x2; V(g1)$y <- V(g1)$y2; E(g1)$weight <- 0.5; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1)
...
#	modes <- c(MEAS_MODE_UNDIR, MEAS_MODE_IN, MEAS_MODE_OUT)
	...
	{	mode <- modes[i]
		...
		V(g)$label <- paste(vertex_attr(g,name=COL_LOC_ID), get.location.names(g),sep="_")
		g1 <- g; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1); g1 <- rescale.coordinates(g1)
		custom.gplot(g=g1, col.att=fname, col.att.cap=MEAS_LONG_NAMES[MEAS_ECCENTRICITY], file=paste0(plot.file,"_lambert"), asp=1, size.att=2, edge.arrow.mode=0, vertex.label.cex=0.1, rescale=FALSE)
		g1 <- g; V(g1)$x <- V(g1)$x2; V(g1)$y <- V(g1)$y2; E(g1)$weight <- 0.5; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1)
