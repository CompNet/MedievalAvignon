...
{	tlog(4,"Computing spatial distance distribution")
	...
	{	tlog(6,"Computing spatial distance (",sdist,")")
		...
		{	satt <- paste0(fname,"_",sdist,"-",avg.type,"_mean")
			...
			custom.gplot(g=g1, col.att=satt, col.att.cap=MEAS_LONG_NAMES[MEAS_DISTANCE_AVG_SPATIAL], file=paste0(plot.file,"_algo"), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
		}
	}
...
{	# get the stat table
	...
	{	tlog(6,"Computing mean type ",avg.type)
		...
		{	plot.file <- file.path(mode.folder,paste0("geodesic","-avg-",avg.type,"_histo"))
			...
			g1 <- g; V(g1)$x <- V(g1)$x2; V(g1)$y <- V(g1)$y2; E(g1)$weight <- 0.5; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1)
			custom.gplot(g=g1, col.att=paste0(fname,"-",avg.type,"_avg"), col.att.cap=MEAS_LONG_NAMES[MEAS_DISTANCE_AVG_GEODESIC], file=paste0(plot.file,"_algo"), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
		}
	}
		...
		{	id <- vertex_attr(g, COL_LOC_ID, n)
			...
			{	g <- set_vertex_attr(graph=g, name=fname, value=vals[n,])
				...
				{	g <- update.node.labels(g, vals[n,])
					...
					g1 <- g; V(g1)$x <- V(g1)$x2; V(g1)$y <- V(g1)$y2; E(g1)$weight <- 0.5; g1 <- delete_edge_attr(g1, LK_TYPE); g1 <- simplify(g1)
					custom.gplot(g=g1, col.att=fname, col.att.cap=MEAS_LONG_NAMES[MEAS_DISTANCE], v.hl=n, file=file.path(mode.folder,"kk",paste0(id.cln,"_",shrt.nm )), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
				}
