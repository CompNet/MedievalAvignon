			...
			{	id <- vertex_attr(g, ND_NAME, n)
				...
				{	g <- set_vertex_attr(graph=g, name=fname, value=vals[n,])
					...
					{	shrt.nm <- substr(nname,1,30)		# to avoid long file names
						...
						custom.gplot(g=g1, col.att=fname, col.att.cap=MEAS_LONG_NAMES[MEAS_STRUCT_SIM], v.hl=n, file=file.path(mode.folder,"lambert",paste0(id.cln,"_",shrt.nm)), asp=1, size.att=2, edge.arrow.mode=0, vertex.label.cex=0.1)
