			...
			{	id <- vertex_attr(g, ND_NAME, n)
				...
				{	g <- set_vertex_attr(graph=g, name=fname, value=vals[n,])
					...
					{	shrt.nm <- substr(nname,1,30)		# to avoid long file names
						...
						custom.gplot(g=g1, col.att=fname, col.att.cap=MEAS_LONG_NAMES[MEAS_STRUCT_SIM], v.hl=n, file=file.path(mode.folder,"kk",paste0(id.cln,"_",shrt.nm)), rescale=FALSE, xlim=range(V(g1)$x), ylim=range(V(g1)$y), edge.arrow.mode=0, vertex.label.cex=0.1, size.att=6)
					}
