...
if(result)
	cat("Crossing detected for (",n1,",",n2,") vs. (",n3,",",n4,")\n",sep="")
			}
...
{	modified <- TRUE
	...
	# repeat as long as there's a modification in the graph
...
#cat("(",n[1],",",n[2],") vs. (",n[3],",",n[4],") => ",modified,"\n",sep="")
					...
					if(modified)
					{	cat("Crossing detected for (",n[1],",",n[2],") vs. (",n[3],",",n[4],")\n",sep="")
...
#print(ni)
								...
								dists <- apply(matrix(es,ncol=2,byrow=TRUE), 1, function(c) dist.nodes(g, c[1], c[2]))
								g <- add.edges(graph=g,edges=es,attr=list(added=TRUE,dist=dists))
							}
							# case of crossing segments
							else
							{	# process the intersection point
								...
								g <- add.vertices(graph=g,nv=1,attr=list(added=TRUE,x=xi,y=yi,sx=0,sy=0,ax=0,ay=0,cx=0,cy=0))
								nc <- vcount(g)
